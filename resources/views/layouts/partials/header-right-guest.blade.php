...
<div class="sm:hidden flex flex-col items-center gap-2 mt-2">
    <x-nav-link href="{{ route('login') }}" :active="request()->is('login')" class="text-sm text-gray-500 hover:text-blue-500">
        Kirjaudu Sisään
    </x-nav-link>
    <x-nav-link href="{{ route('register') }}" :active="request()->is('register')" class="text-sm text-gray-500 hover:text-blue-500">
        Liity
    </x-nav-link>
</div>
